<?php
	...
	if (!empty($_POST["VisitorName"]) && !empty($_POST["Email"]) && trim((string) ($_POST["VisitAbout"] ?? "")) !== "") {
		...
		if ($_POST["VisitStatus"]!=3) $_POST["CheckoutTime"] =  "";
		...
		if (empty($_POST["VisitID"])) {  // Add Visits
			//echo "Add visit";
			$exclude = Array("VisitID", "RoomTimeID", "EntryDateTime","chkosystem", "ModifDateTime", "ipad", "CourseTimeID");
			...
			//echo "Edit visit";
			$exclude = Array("RoomTimeID", "EntryDateTime","chkosystem", "ModifDateTime", "ipad", "CourseTimeID");
	...
	if (!empty($_GET["vid"])) { // visitID
	  // Display data
		...
		if ($vData[0]["CheckoutTime"] === "00:00:00" || empty($vData[0]["CheckoutTime"])) { $checkoutTime = date('H:i',strtotime('+1 hour',strtotime($vData[0]["CheckinTime"]))); $checkoutTime = date("g:i A", strtotime($checkoutTime)); }
	} else { 
...
<script>
	jQuery().ready(function($){
		...
		//only numbers
		$('#BadgeID').bind('keypress', function (e) {  return (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) ? false : true; });
		...
		$("#submitdatav").click(function () {				
			...
			$('#BadgeID').css({"border-color": "#bdbdbd", "background-color": "#FFF"});
			...
			if ($.trim($("#Email").val()) == "") { 
				err=1; $('#Email').css({"border-color": "red", "background-color": "#F79BB7"});  
			} else { 
				var email1 = $.trim($("#Email").val()); 
				if(!emailReg.test(email1) ) {
					$("#Email").focus(); em=1; $('#Email').css({"border-color": "red", "background-color": "#F79BB7"}); 				
				} 
			}
				...
				jQuery.ajax({
				  ...
				  success: function(msg) {							
						...
						if (msg!="") {
							...
							if (res[2]!="" && res[2]!="0") { /*alert(msg+": Misy3");*/ msgr+="Badge# is not available, please type another badge #"; $('#BadgeID').css({"border-color": "red", "background-color": "#F79BB7"});} 
		...
		.autocomplete( "instance" )._renderItem = function( ul, item ) {
		  return $( "<li>" )
			...
			.append( "<div>" + item.value + " - <span class='text-bold'>" + item.email + "</span></div>" )
			.appendTo( ul );
		};
    ...
    // Reset field styles
    $("#Email, #VisitorName, #Mobile, #CompanyName, #BadgeID, #VisitAbout, #VisitComment, #HostID, #AreaID, #VisitStatus")
    ...
    $("#errormsg").html("");
		...
		$("#VisitorName").change(function( ) {					 
			...
			if (anarana != $(this).val()) { 
				if (anarana!="") { $("#VisitorID").val(""); $("#Email").val(""); $("#Mobile").val(""); $("#Email").prop('disabled', false); $("#Mobile").prop('disabled', false);}
				anarana = $(this).val(); 
			}			
		}); */
...
<section class="panel">
		...
		<div class="panel-body">					
			<div class="alert alert-info">
				<p class="m-none text-semibold h6">For an existing visitor, type 2 or more characters of his/her name in the "Visitor Name" field below, then <u><span class="text-danger">SELECT</span></u> it to display his/her existing info in the system.<br><br><span class="text-danger">Please separate Visitor First or Middle Name and Last Name with a space.</span></p>
			</div>
			...
			<form name="fvisit" id="fvisit" method="post">
				<input type="hidden" name="VisitorID" id="VisitorID" value="<?php echo (empty($vData[0]["VisitorID"])) ? "" : $vData[0]["VisitorID"]; ?>" />
				<input type="hidden" name="VisitID" id="VisitID" value="<?php echo (empty($vData[0]["VisitID"])) ? "" : $vData[0]["VisitID"]; ?>" />
				<input type="hidden" name="VisitorType" id="VisitorType" value="<?php echo (empty($vData[0]["VisitorType"])) ? "GENERAL" : $vData[0]["VisitorType"]; ?>" />
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							<label class="col-md-3 control-label" for="inputDefault">Visitor Name <span class="required">*</span></label>
						<div class="col-md-6">
							<input type="text" class="form-control" id="VisitorName" name="VisitorName" placeholder="First/Middle name  Last name" value="<?php echo (empty($vData[0]["VisitorName"])) ? "" : $vData[0]["VisitorName"]; ?>">
							<span class="help-block text-danger" id="visitorNameError" style="display:none; margin-top: 5px;">
					...
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="form-control" id="CompanyName" name="CompanyName" value="<?php echo (empty($vData[0]["CompanyName"])) ? "" : $vData[0]["CompanyName"]; ?>">
							</div>
						</div>
					</div>
				</div><br>
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="form-control" id="Email" name="Email" value="<?php echo (empty($vData[0]["Email"])) ? "" : $vData[0]["Email"]; ?>">
							</div>
						</div>						
					</div>
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<div class="input-group">
									...
									<input type="text" data-plugin-masked-input data-input-mask="(999) 999-9999" placeholder="(123) 123-1234" class="form-control" id="Mobile" name="Mobile" value="<?php echo (empty($vData[0]["Mobile"])) ? "" : $vData[0]["Mobile"]; ?>">
								</div>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="form-control" id="CenterIDDisplay" value="<?php echo $_SESSION['center']; ?>" readonly style="background-color: #f5f5f5; cursor: not-allowed;">
								<input type="hidden" name="CenterID" id="CenterID" value="<?php echo $_SESSION['id_center']; ?>">
							</div>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="form-group">
							<label class=" col-md-3 control-label">QS Center Area</label>
							<div class="col-md-6">
								<select class="input-sm" id="AreaID" name="AreaID">
									<?php echo ListCenterAreas($pdo, (empty($vData[0]["AreaID"])) ? 0 : $vData[0]["AreaID"], " where CenterID =".$pdo->quote($_SESSION["id_center"])); ?>
								</select>
							</div>
						</div>
					</div>
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-2">
								<input type="text" class="form-control" id="BadgeID" name="BadgeID" maxlength="8" value="<?php echo (empty($vData[0]["BadgeID"])) ? "" : $vData[0]["BadgeID"]; ?>">
							</div>
						</div>
					</div>
				</div><br>
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="inp-date" id="VisitDate" name="VisitDate" value="<?php echo date("m/d/Y");?>" readonly="true">
							</div>
						</div>
					</div>
					<!-- <div class="col-sm-6">
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<div class="input-group">								
									...
									<input id="CheckinTime" name="CheckinTime" type="text" data-plugin-timepicker class="inp-tim" value="<?php echo  (empty($vData[0]["CheckinTime"])) ? date('g:i A', time()) : date("g:i A", strtotime($vData[0]["CheckinTime"])); ?>" data-plugin-options='{ "minuteStep": 1 }' readonly="true">
								</div>
							</div>
						</div>
					</div>
					...
					<div class="col-sm-6">
						<div class="form-group" id="sec_chkotime">
							...
							<div class="col-md-4">
								...
								<input id="CheckoutTime" name="CheckoutTime" type="text" data-plugin-timepicker class="inp-tim" value="<?php echo (empty($vData[0]["CheckoutTime"])) ? date('g:i A', time()) : date("g:i A", strtotime($vData[0]["CheckoutTime"])); ?>" data-plugin-options='{ "minuteStep": 1 }' readonly="true">
								</div>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<textarea class="form-control frmss" id="VisitAbout" name="VisitAbout"><?php echo (empty($vData[0]["VisitAbout"])) ? "" : $vData[0]["VisitAbout"]; ?></textarea>
								<span class="help-block text-danger" id="visitAboutError" style="display:none; margin-top: 5px;">
					...
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="col-md-6">
								<textarea class="form-control frmss" id="VisitComment" name="VisitComment"><?php echo (empty($vData[0]["VisitComment"])) ? "" : $vData[0]["VisitComment"]; ?></textarea>
							</div>
						</div>
					</div>
				</div><br>
